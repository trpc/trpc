...
export const postsRouter = router({
  ...
  list: publicProcedure.meta({ authRequired: false }).query(() => {
    return getPosts();
  }),
...
export const appRouter = t.router({
  // Public endpoint
  hello: authProcedure.meta({ authRequired: false }).query(() => 'Hello world'),
...
const baseProcedure = publicProcedure.meta({
  authRequired: true,
  role: 'user',
});

const adminProcedure = baseProcedure.meta({
