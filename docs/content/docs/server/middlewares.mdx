...
const loggerMiddleware = t.middleware(({ path, type, next }) => {
  ...
  return next().finally(() => {
    ...
    console.log(
      `⬅️ ${type.toUpperCase()} ${path} - Completed in ${duration}ms`,
    );
  });
});
...
// Usage
export const rateLimitedProcedure = t.procedure.use(rateLimit(10, 60000)); // 10 requests per minute
```
...
export const adminProcedure = fullProcedure.use(requireRole(['admin']));

export const organizationProcedure = fullProcedure.use(withOrganization);

export const transactionalOrganizationProcedure =
  organizationProcedure.use(withTransaction);
```
...
export const betaFeatureProcedure = t.procedure.use(
  withFeatureFlag('beta-features'),
);
export const adminIPProcedure = t.procedure.use(
...
const auditedAdminAuth = adminAuth.unstable_pipe(
  async ({ ctx, path, next }) => {
    // Log admin action
    await auditLog({
      userId: ctx.user.id,
...
describe('Authentication Middleware', () => {
  ...
  it('should enforce role-based access', async () => {
    ...
    await expect(userCaller.admin.deleteUser({ id: 'user2' })).rejects.toThrow(
      'FORBIDDEN',
    );
  });
});
...
describe('Rate Limiting Middleware', () => {
  ...
  it('should reject requests exceeding limit', async () => {
    ...
    await expect(caller.rateLimited.hello()).rejects.toThrow(
      'TOO_MANY_REQUESTS',
    );
  });
});
