...
export const organizationMemberProcedure = protectedProcedure.use(
...
const authorize = (action: string) => {
  return t.middleware(async ({ ctx, input, next }) => {
    ...
    let resource = null;
    const resourceId =
      (input as any)?.id || (input as any)?.postId || (input as any)?.userId;
    if (resourceId) {
...
// Usage
export const readPostProcedure = protectedProcedure.use(
  authorize('posts:read'),
);
export const editPostProcedure = protectedProcedure.use(
  authorize('posts:edit'),
);
export const deleteUserProcedure = protectedProcedure.use(
...
export const orgAdminProcedure = protectedProcedure.use(
  withOrganizationAccess('admin'),
);
export const orgOwnerProcedure = protectedProcedure.use(
  withOrganizationAccess('owner'),
);
...
export const sessionAuth = t.middleware(async ({ ctx, next }) => {
  const sessionId =
    ctx.req.headers['x-session-id'] || ctx.req.cookies?.sessionId;
...
describe('Authorization', () => {
  ...
  describe('Protected endpoints', () => {
    it('should reject unauthenticated requests', async () => {
      ...
      await expect(caller.protected.getProfile()).rejects.toThrow(
        'UNAUTHORIZED',
      );
    });
