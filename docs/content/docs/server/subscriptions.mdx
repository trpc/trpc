...
export function PostList() {
  ...
  return (
    <div>
      {posts.map((post) => (
        <div key={post.id} className="mb-2 rounded border p-4">
          <h3>{post.title}</h3>
          <p>{post.content}</p>
        </div>
      ))}
...
export const liveRouter = t.router({
  onChannelMessage: t.procedure
    ...
    .subscription(async function* (opts) {
      ...
      if (lastEventId) {
        const missedMessages = await getChannelMessagesSince(
          channelId,
          lastEventId,
        );
        for (const message of missedMessages) {
...
export async function* batchEvents<T>(
  ...
  for await (const event of eventStream) {
    ...
    const shouldFlush =
      batch.length >= batchSize || Date.now() - lastFlush >= maxWaitMs;
