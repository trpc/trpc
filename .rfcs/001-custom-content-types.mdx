...
This RFC is proposes to add the ability to return a `Response` object from procedures. This is useful when you need full control over the response, such as returning files, streams, or custom headers.
...
const appRouter = router({
  hello: publicProcedure
    .output('Response') // <-- define that we're returning a Response
    .query(async () => {
...
- The `.output('Response')` is static because it changes the type of the procedure in a way that is not compatible with the usual tRPC type system and allows us to reject a request if it is tried to be called with e.g. batching.
...
export const appRouter = t.router({
  ...
  downloadFile: publicProcedure.output('Response').query(() => {
    return new Response('Hello World', {
      headers: {
