...
Suggestion to add the ability to return a `Response` object from your procedure by specifying `'Response'` as the output type. This is useful when you need full control over the response, such as returning files, streams, or custom headers.
...
const appRouter = router({
  hello: publicProcedure.output('Response').query(() => {
...
The client will not be able to handle the `Response` type directly. You will need to use the `httpLink`.
...
// Usage
const res = await client.downloadFile.query();
//    ^? Response
```
