...
- The `.output('Response')` is static because it changes the type of the procedure in a way that is not compatible with the usual tRPC type system and allows us to reject a request upfront if it is tried to be called with e.g. batching.
