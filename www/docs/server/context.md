...
The `createContext()` function must be passed to the handler mounting your `appRouter`. HTTP handlers and [WebSocket](websockets.md#create-context) handlers invoke it for you. The **returned context** is also used by [server-side calls](server-side-calls): `createCaller` and `createServerSideHelpers` consume that context object directly — they do not pass `createContext()` to a handler.
