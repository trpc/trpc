...
if (
  error.shape &&
  'reason' in error.shape.data &&
  error.shape.data.reason === 'BAD_PHONE'
) {
  error.shape.data.reason;
}
...
- Registered declared errors are available on the client via `error.shape.data`.
- If you want ergonomic frontend narrowing for declared errors, include a literal discriminator such as `reason` or `kind` in the declared data.
...
- Downgraded declared errors use the normal formatted client path, so formatter data is available via `error.data`.
