...
const client = createTRPCClient<AppRouter>({
  links: [
    splitLink({
      ...
      false: httpBatchLink({
        // Standalone `createHTTPServer` only (no extra path).
...
The `httpBatchLink` URL must match the HTTP handler:

- **Standalone** (`createHTTPServer` below, no extra path): `http://localhost:3000`
- **Express** (mounted at `/trpc` later in this page): `http://localhost:3000/trpc`
...
With Express, wrap the app in Node's HTTP server and attach `ws` the same way. Use a **separate** `createContext` for each adapter — see [`createContext` typing](#create-context).
...
:::tip
If you're doing a web application, you can ignore this section as the cookies are sent as part of the WebSocket upgrade request. Reading those cookies (and why Express `req.cookies` is not available on the WS handler) is covered in [`createContext` typing](#create-context).
:::
...
## `createContext` typing {#create-context}
