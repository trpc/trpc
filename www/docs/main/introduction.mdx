...
### tRPC explained from a REST API background (Express)

tRPC can co-exist with a REST API, as it uses a single endpoint. On this specific endpoint tRPC will itself function as a router, meaning tRPC will redirect internally to the right handler on every incoming request. From this point of view, `@trpc/server` can be seen as router module you could use on top of the framework.
...
tRPC is for full-stack TypeScript developers. It makes it easy to write endpoints which you safely use in your app.
