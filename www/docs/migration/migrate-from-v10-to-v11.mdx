...
The standalone adapter now supports a `pathname` option, which will slice the pathname from the beginning of the request path.

See the [standalone adapter docs](../server/adapters/standalone.md#custom-pathname) for more information.
