...
### subscription improvements (non-breaking)

Added a `onStateChange` callback on the client for subscriptions. This callback can be used to track the state of the subscription.

- `onError` has been deprecated in favor of `onStateChange`
