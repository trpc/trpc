...
### Minor updates to `unstable_httpBatchStreamLink` response format (semi-breaking)

No changes on actual usage, but the response format has been updated to return valid JSON-values (in `11.0.0-rc.644`)

If you use this link and have a lot of users actively using your application, you might want to do a change where you first change to use `httpBatchLink` and deploy, then change to use `unstable_httpBatchStreamLink` while you upgrade you app and then deploy again.
...
### TypeScript version >=5.6.2 is now required (non-breaking)
