...
### Simplified `next()`-calls in middlewares (non-breaking)

We have simplified the `next()`-calls in middlewares, this means that you don't need to use the `ctx`-key on the `next()` anymore, simply call `next()` with an object to override the context.

**Before**

```ts
const myProcedure = publicProcedure.use(async (opts) => {
  // [...]
