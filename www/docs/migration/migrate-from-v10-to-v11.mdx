...
### `resolveHTTPRequest` has been replaced by `resolveRequest` that uses Fetch APIs (breaking for HTTP-adapters)
...
### `TRPCRequestInfo.input` has been removed (breaking)

Inputs are now materialised lazily when required by the procedure, which means the input is no longer available when tRPC calls `createContext`

### All the experimental form-data support has been replaced (breaking)
...
You can see the new approach in `examples/minimal-content-types`
...
### NodeJS 18+ and Modern Browsers are now required (breaking)
