...
### Removed `@trpc/client` exports (non-breaking)

We have removed the `@trpc/client` exports from the `@trpc/react-query` package.

This should not be a breaking change, but if you have missing `import`s, you might need to change some from `@trpc/react-query` to `@trpc/client`.
