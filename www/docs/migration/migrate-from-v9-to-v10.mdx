...
See [Inferring types](/docs/server/infer-types) for more.
...
whole routers](/docs/reactjs/usecontext#invalidating-across-whole-routers), we needed to change how we use tanstack query keys under the
hood.
...
tRPC is no longer exposing the `queryClient` instance through `trpc.useContext()`. If you need to use some methods from `queryClient`, check if `trpc.useContext()` wraps them [here](/docs/reactjs/usecontext#helpers). If tRPC doesn't wrap the respective method yet, you can import the `queryClient` from `@tanstack/react-query` and use it that way:
