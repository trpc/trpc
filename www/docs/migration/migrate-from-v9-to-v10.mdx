...
```ts twoslash
import trpc, { initTRPC, TRPCError } from '@trpc/server';
...
const appRouter = t.router({
  greeting: authedProcedure.query(({ ctx }) => `hello ${ctx.user.name}!`),
});
```
...
All you'll need to do is to add an `.interop()` at the end of your `appRouter`:
...
const legacyRouter = trpc.router()
  /* .... */
  .interop(); // <--- Add this
...
```ts
export const appRouter = t.merge(legacyRouter, mainRouter);
```
