...
4. Create a test router:

```diff title="/src/routers/greeting.ts"
...
Any procedures defined using `t.procedure` must be called on the client using the [new way of calling procedures](#calling-procedures) and vice-versa for old procedures. This means you have to move over an entire router on both the backend and frontend at once.
...
As you can see, we no longer have to setup the react-query hooks and Next.js separately. The `trpc` object here contains all you need for your frontend application.
