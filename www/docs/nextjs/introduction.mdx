...
We recommend a file structure like this one but it is not enforced by tRPC. This is what you'll get when starting from [the examples](/main/example-apps.md).
...
- [@tanstack/react-query](https://tanstack.com/query/v4/docs/adapters/react-query): `@trpc/react-query` provides a thin wrapper around react-query. It is required as a peer dependency as an idiomatic way to handle client-side caching in React applications.
- [Zod](https://github.com/colinhacks/zod): We will use Zod for input validation to ensure that our backend only processes requess that fit our API. You can use other validation libraries like ([Yup](https://github.com/jquense/yup), [Superstruct](https://github.com/ianstormtaylor/superstruct), [io-ts](https://github.com/gcanti/io-ts) if you prefer.
...
:::note
The backend above is using the [recommended file structure](#recommended-file-structure), but you can keep it simple and put everything in [an API handler directly](https://github.com/trpc/trpc/blob/next/examples/next-minimal-starter/src/pages/api/trpc/%5Btrpc%5D.ts) if you prefer.
:::
...
Browse the rest of the docs to learn more about things like [authorization](/docs/authorization), [middlewares](/docs/middlewares), and [error handling](/docs/error-handling).

You can also find usual information about [queries](react-queries) and [mutations](react-mutations) now that you're using `@trpc/react-query`.
