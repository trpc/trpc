...
Initialize your tRPC backend in `src/server/trpc.ts` using the `initTRPC` function, and create your first router. We're going to make a simple "hello world" router and procedure here - but for deeper information on creating your tRPC API you should refer back to the [quickstart guide](/docs/quickstart) and the [Backend usage docs](/docs/server/introduction)
