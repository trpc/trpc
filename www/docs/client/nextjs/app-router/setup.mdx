...
:::note
The prefetching examples below use `queryClient.query()` and `queryClient.infiniteQuery()`, which require `@tanstack/react-query@5.102.0` or newer.
:::
...
guide, you can use `query` instead of `prefetch` to have the data both on the server as well as hydrating it down to the client:
...
export default async function Home() {
  const queryClient = getQueryClient();
  const greeting = await queryClient.query(trpc.hello.queryOptions());
