...
If you want to prefix all queries and mutations with a specific key (for example, to namespace them by user ID), you can enable the `enablePrefix` feature flag when creating the tRPC context.

```tsx title='components/App.tsx'
import { createTRPCContext } from '@trpc/tanstack-react-query';
import type { AppRouter } from '../server/router';

// Enable the prefix feature flag
  ...
  return (
    <QueryClientProvider client={queryClient}>
      <TRPCProvider
        ...
        // Still optional but your types will now support it
