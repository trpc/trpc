...
export default function Basics() {
  ...
  const queryOptions = trpc.path.to.query.queryOptions({ /** inputs */ })
  // useQuery(queryOptions), useSuspenseQuery(queryOptions), useInfiniteQuery(queryOptions)
...
```ts
const queryOptions = trpc.path.to.query.queryOptions({ /** input */ }, {
...
```ts
const infiniteQueryOptions = trpc.path.to.query.infiniteQueryOptions({ /** input */ }, {
  // Any Tanstack React Query options
...
Available for all query procedures as well as routers and sub-routers. Allows you to access the query key in a type-safe manner.
...
```ts
const queryFilter = trpc.path.to.query.queryFilter({ /** input */ }, {
