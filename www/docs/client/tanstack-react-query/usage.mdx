...
```tsx
// utils/trpc.ts
export const { TRPCProvider as BillingTRPCProvider, useTRPC as useBilling } = createTRPCContext<BillingRouter, { keyPrefix: true }>();
export const { TRPCProvider as AuthTRPCProvider, useTRPC as useAuth } = createTRPCContext<AuthRouter, { keyPrefix: true }>();
...
export function App() {
  ...
  return (
    <QueryClientProvider client={queryClient}>
      <BillingTRPCProvider trpcClient={billingClient} queryClient={queryClient} keyPrefix="billing">
        <AuthTRPCProvider trpcClient={authClient} queryClient={queryClient} keyPrefix="auth">
          <MyComponent />
        </AuthTRPCProvider>
      </BillingTRPCProvider>
    </QueryClientProvider>
...
function MyComponent() {
  ...
  const billList = useQuery(billing.list.queryOptions());
  const authList = useQuery(auth.list.queryOptions());
