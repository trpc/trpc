...
```ts
const queryKey = trpc.router.routeKey();
```

Or even the root path to match all tRPC queries:

```ts
const queryKey = trpc.routeKey();
```
...
Like with query keys, if you want to run a filter across a whole route you can use `routeFilter`

```ts
const queryFilter = trpc.path.routeFilter({
  // Any Tanstack React Query filter
  predicate: (query) => {
    query.state.data;
  },
