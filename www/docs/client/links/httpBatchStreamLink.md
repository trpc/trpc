...
- Cause the requests to be sent with a `trpc-accept: application/jsonl` header (or an `accept=application/jsonl` query parameter when using `streamIndicator: 'query'`)
