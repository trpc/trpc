...
### Route subscriptions over WebSockets

Use `splitLink` so queries and mutations stay on HTTP while subscriptions use [`wsLink`](./wsLink.md). See [Using HTTP and WebSockets together](../../server/websockets.md#http-and-websockets) for the matching server setup.
