...
The `localLink` is a special link that allows you to make tRPC procedure calls directly in your application without going through HTTP. This is particularly useful in scenarios where you want to run the router in the same process as the client, such as in server components or during development and testing.
...
- **SSR (Server-Side Rendering)**: Make direct procedure calls during server-side rendering
- **Testing**: Test your client code with the actual server implementation
...
- When using with server components, make sure to handle any server-only code appropriately
