...
:::tip
If you're using a transformer like `superjson`, you probably want React Query to be able to use [structural sharing](https://tanstack.com/query/latest/docs/framework/react/guides/render-optimizations#structural-sharing) for non-JSON types like Date, Set and Map. You can do this by setting the `structuralSharing` option to `replaceEqualDeep` in the `queryClient` options.
