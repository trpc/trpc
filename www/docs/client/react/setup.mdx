...
If you're using a [transformer](../../server/data-transformers.md) like `superjson`, you probably want this to work for non-JSON types like Date, Set and Map. You can do this by setting the `structuralSharing` option to `replaceEqualDeep` in the `queryClient` options.
