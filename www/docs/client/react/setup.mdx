...
### 4. Add tRPC providers {#add-trpc-providers}
...
:::tip
React Query uses a technique called [structural sharing](https://tanstack.com/query/latest/docs/framework/react/guides/render-optimizations#structural-sharing) to ensure that as many references as possible will be kept intact between re-renders.

If you're using a [transformer](../../server/data-transformers.md) like `superjson`, you probably want this to work for non-JSON types like Date, Set and Map. You can do this by setting the `structuralSharing` option to `defaultStructuralSharingFunction` in the `queryClient` options.
