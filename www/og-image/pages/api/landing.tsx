...
const fetchNpmDownloads = async () => {
  const res = await fetch(
    'https://api.npmjs.org/downloads/point/last-week/@trpc/server',
  );
  const data = await res.json();
...
const fetchTwitterFollowers = async () => {
  ...
  if (typeof json?.data?.public_metrics?.followers_count !== 'number')
...
export default async (_req: Request) => {
  const [inter800, inter700, starCount, npmDownloads, twitterFollowers] =
    await Promise.all([
          ...
          <div tw="flex items-center text-zinc-300">
            <div tw="flex items-center mx-8">
              <div tw="flex items-center mx-8">
                ...
                <p tw="text-3xl font-bold ml-2">{starCount}</p>
              </div>
