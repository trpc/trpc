...
tRPC has always followed the [SemVer](https://semver.org/) versioning scheme, but honestly this is hard work with little benefit to the way that tRPC releases work, and our time on the `@next` tag has taught us a way we prefer. Going forward we'll continue to use major version numbers for breaking changes, but we'll only have a single incrementing number after this. This means that patches and enhancements will both be treated equally, allowing us to release with a faster cadence as improvements come ready. For you this means you don't need to consider minor/patch versions, we'll always aim for 0 breaking changes during a major version so you just update to the latest release on your major.
