...
**With the launch of tRPC v11**, we're excited to share that v11 is already being used in production by many large TypeScript projects thanks to its stable evolution on our `@next` channel. This official release marks the end of an exciting development cycle which brings support for Tanstack Query v5, Next.js App Router, SSE-based subscriptions, and plenty of other goodies.
...
## Changes in how we release tRPC

With tRPC v11 we're going to change how we do versioning going forward.

tRPC has always followed the [SemVer](https://semver.org/) versioning scheme, but honestly this is hard work with little benefit to the way that tRPC releases work, and our time on the `@next` tag has taught us a way we prefer. Going forward we'll continue to use major version numbers for breaking changes, but we'll have a single incrementing minor version. This means that patches and enhancements will both be treated equally, allowing us to release with a faster cadence as improvements are ready. For you this means you don't need to consider minor/patch versions, and we'll always aim for 0 breaking changes during a major version so you can always upgrade with confidence.
...
While you have been able to use tRPC with Next.js App Router from day 1, either:
...
the bridge in between has been a bit rough around the edges. Revalidation patterns are mixed when having to maintain different data fetching patterns resulting in a less than ideal developer experience that does not live up to tRPC's standards.

To fix this, we've improved support for React Server Components (RSC) and added prefetch helpers to make it easier to utilize the powers of RSCs running exclusively on the server, in combination with the highly dynamic client-side cache of React Query.
You can now start executing a procedure on the server in a RSC, and pick up the pending promise on the client and automatically hydrate the React Query cache on the client. This enables you to build hihgly dynamic applications without suffering from
server-client waterfalls. You can read more in the [Server Components documentation](/docs/client/react/server-components).

In addition to these new prefetch patterns, we've added experimental support for server functions. You can read more about this in [our previous blog post](/blog/trpc-actions). We plan to keep iterating on this feature as server functions become a more established
pattern in the ecosystem. We also worked with the [TanStack team to help design the APIs for their server functions](https://x.com/tannerlinsley/status/1844500352655335522).
Our goal is to split out parts of tRPCs highly appreciated middleware system into a separate package that can be used throughout the ecosystem.
