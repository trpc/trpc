...
Let's assume we have simple tRPC router with three procedures that looks like this:
...
const appRouter = router({
  post: router({
    ...
    byTitle: publicProcedure
      ...
      .query(({ input }) => {
        return posts.find((p) => p.title === input.title);
      }),
...
What we want to do is to map the `post.byId.query()` to a GET request to our server, and `post.create.mutate()` to a POST request, and the types should all be propagated from back to front . So, how do we do this?
...
We'll need to use recursive types so that we can infer arbitrary deep router structures. Also, we know that we want our procedures `post.byId` and `post.create` to have the `.query` and `.mutate` on them respectively - in tRPC, we call this decorating the procedures. In `@trpc/server`, we have some inference helpers that will infer the input and output types of our procedures with these resolved methods, which we'll use to infer the types for these functions, so let's write some code!

The first thing we'll define is the type for our resolver functions (`query` and `mutate`):
...
Finally, we'll create a type that will recursively traverse the router and decorate all sub-routers and procedures along the way:

```twoslash include decoraterecord
import type { AnyRouter, ProcedureRouterRecord } from "@trpc/server";
...
💻 This is the exact implementation we have in the tRPC codebase today, [see for yourself](https://github.com/trpc/trpc/blob/minitrpc/packages/server/src/shared/createProxy/index.ts)!
...
function createRecursiveProxy(callback: ProxyCallback, path: string[]) {
  const proxy: unknown = new Proxy(
    {
      // dummy object since we don't have any client-side target
      // we want to remap to, we just want to intercept the calls
...
I hope you enjoyed this article and learned something about how tRPC works. You should probably not use this to in favor of the @trpc/client which is only a couple of kbs bigger - it comes with a lot more flexibility than what we're showcasing here:
...
We also didn't cover much of the server side of things today, maybe we'll cover that in a future article.
