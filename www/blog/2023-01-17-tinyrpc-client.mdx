...
import type { AnyRouter, AnyProcedureRouterRecord } from "@trpc/server";
...
1. We pass a `AnyProcedureRouterRecord` to the type as a generic, which is a type containing all the procedures and sub-routers that exists on a tRPC router.
