...
We store a list of company logos in the `www/static/img/logos/*` folder to show on the landing page. To add future logos, simple add the png/svg to the folder, and then add it to the `www/src/components/companies.ts` map with the key of the company name and the value of the logo file location. Be sure to normalize your logos, and ensure that it has a set width and height.
