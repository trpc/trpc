...
type inferProcedures<TObj extends ProcedureRecord<any, any, any, any>> = {
  [TPath in keyof TObj]: {
    ...
    handlerInput: inferHandlerInput<TObj[TPath]>;
  };
};

export type MakeRouter<TRouter extends AnyRouter> = {
  TQueryValues: inferProcedures<TRouter['_def']['queries']>;
  TMutationValues: inferProcedures<TRouter['_def']['mutations']>;
  TError: TRPCClientErrorLike<TRouter>;
  TInfiniteQueryNames: inferInfiniteQueryNames<TRouter['_def']['queries']>;
  TRouter: TRouter;
};

export function createReactQueryHooks2<TQuick extends MakeRouter<any>>() {
  type TRouter = TQuick['TRouter'];
  type TSubscriptions = TRouter['_def']['subscriptions'];
  type TError = TRPCClientErrorLike<TRouter>;
  type TInfiniteQueryNames = TQuick['TInfiniteQueryNames'];

  type TQueryValues = TQuick['TQueryValues'];
  type TMutationValues = TQuick['TMutationValues'];
  ...
  function useQuery<TPath extends keyof TQueryValues & string>(
    pathAndInput: [path: TPath, ...args: TQueryValues[TPath]['handlerInput']],
