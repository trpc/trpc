...
interface TRPCMutationOptionsIn<TInput, TError, TOutput, TContext>
  extends DistributiveOmit<
      SolidMutationOptions<TOutput, TError, TInput, TContext>,
      ReservedOptions
    >,
    TRPCQueryBaseOptions {}

interface TRPCMutationOptionsOut<TInput, TError, TOutput, TContext>
  extends SolidMutationOptions<TOutput, TError, TInput, TContext>,
    TRPCQueryOptionsResult {
  ...
  return {
    ...
    onSuccess(data: unknown, variables: unknown, context: unknown) {
      const originalFn = () =>
        ...
        defaultOpts?.onSuccess?.(data, variables, context);

      return mutationSuccessOverride({
        originalFn,
        queryClient,
        meta: defaultOpts?.meta ?? {},
      });
    },
