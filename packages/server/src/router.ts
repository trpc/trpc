...
export type Route<TContext = unknown, TInput = unknown, TOutput = unknown> = (
  ...
  _middlewares?: MiddlewareFn<TContext>[];
};
...
export class Router<
  ...
  TMiddlewareFn extends MiddlewareFn<TContext>
> {
  ...
  preHook(fn: PreHookFunction<TContext>) {
    this._def.middlewares.push(async (next, opts) => {
      await fn(opts);
      await next();
    });
    return this;
...
export function router<TContext>() {
  return new Router<TContext, {}, {}, {}, MiddlewareFn<TContext>>();
}
