...
function createAppRouter() {
  ...
  const trpc = trpcReact<typeof appRouter>();
...
test('useLiveQuery()', async () => {
  const { trpc: hooks, db, postLiveInputs } = factory;
  function MyComponent() {
...
test('dehydrate', async () => {
  const { trpc: hooks, appRouter, db } = factory;
...
test('prefetchInfiniteQuery()', async () => {
  const { trpc: hooks, appRouter } = factory;
  const ssr = hooks.ssr(appRouter, {});
...
describe('invalidate queries', () => {
  test('queryClient.invalidateQueries()', async () => {
    const { trpc: hooks, resolvers } = factory;
    function MyComponent() {
...
test('"caller" is of correct type', async () => {
  const { appRouter, trpc: hooks } = factory;
  const ssr = hooks.ssr(appRouter, {});
